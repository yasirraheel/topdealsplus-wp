...
import {
	syncBarItems,
	getFetchDescriptorFor as getBarFetchDescriptorFor,
	drawCompareBar,
} from './bar'
...
const syncBarItemsCallback = (data) => {
	...
	;[...document.querySelectorAll(selector)].map((el) => {
		...
		if (allCompareItems.indexOf(getProductIdFromElement(el)) > -1) {
		...
		syncCompareProductsState({
			...
			cb: () => {
				...
				if (
					...
					getBarFetchDescriptorFor()
						.fetch()
						.then((data) => {
							completeAction({
								finalState,
								onCompleted: () => {
									drawCompareBar(data.content)
									syncBarItemsCallback(data)
								},
							})
						})
