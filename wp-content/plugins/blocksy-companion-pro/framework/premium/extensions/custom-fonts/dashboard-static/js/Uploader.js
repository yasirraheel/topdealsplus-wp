...
export const SingleVariation = ({
	...
	return (
		<li>
			...
			<button
				className="ct-ui-button ct-upload-edit-font-button"
				onClick={() => {
					let frame = wp.media({
						button: {
							text: 'Select',
...
const Uploader = ({
	...
	const futureFont =
		futureFontGlobal[currentFontType] || getDefaultFutureFont()
	...
	return (
		<div
			...
			{!isEditMode ? (
				<h2>{__('Add Fonts', 'blocksy-companion')}</h2>
			) : (
			...
			<div className="ct-modal-actions has-divider">
				<button
					...
					disabled={isDisabled}
					onClick={() => {
						if (currentFontType === 'stack') {
							onChange({
								...customFontsSettings,
								stacks: pickedStackFonts,
							})
